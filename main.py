...
# Bot configuration
TELEGRAM_TOKEN = "7348550676:AAELTZex6rzTVOsVORDsvWu2_cTgd_PXi3Y"
OWNER_ID = 5696226098
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_time - last_check_time < 10:
        ...
        return
    if not context.args or len(context.args) > 1:  # Allow only one card at a time
...
async def env(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if not context.args:
        await update.message.reply_text("Usage: /env [topic]\nExample: /env weather or /env generate cat")
        return
    topic = " ".join(context.args).lower()
    if "generate" in topic:
        prompt = topic.replace("generate", "").strip()
        if prompt:
            await update.message.reply_text(f"Do you want to generate an image for '{prompt}'? Reply with 'yes' to confirm.")
            context.user_data['pending_image'] = prompt
        else:
            await update.message.reply_text("Please specify what to generate, e.g., /env generate cat")
    else:
        # Simple info response (expandable with web search or X analysis if needed)
        info = f"Info about {topic}: This is a basic response. Let me know if you want more details!"
        await update.message.reply_text(info)

async def handle_image_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    if 'pending_image' in context.user_data and update.message.text.lower() == 'yes':
        prompt = context.user_data['pending_image']
        del context.user_data['pending_image']
        await update.message.reply_text(f"Generating image for '{prompt}'... (Note: Image generation is simulated here, implement with an API like DALL·E if desired).")
        # Placeholder for image generation (ask for confirmation instead of generating directly)
        # await context.bot.send_photo(chat_id=user_id, photo=generated_image)
    elif 'pending_image' in context.user_data:
        del context.user_data['pending_image']
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_image_confirmation))  # Handle 'yes' for image
